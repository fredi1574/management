...
import { Plus, Pencil, Trash2, X, TrendingUp, Calendar as CalendarIcon, Tag, Wallet, Info } from "lucide-react";
    ...
    <div className="rounded-3xl glass p-6 shadow-xl space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-zinc-500 pl-12">Monitor your earnings</p>
        </div>
        <motion.button
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5 ml-1">
                  <Wallet className="h-3 w-3" /> Description
                </label>
                <input name="notes" type="text" placeholder="Source of income" className="w-full rounded-xl border-none bg-white px-4 py-2.5 shadow-sm ring-1 ring-zinc-200 focus:ring-2 focus:ring-emerald-500 dark:bg-zinc-900 dark:ring-zinc-700" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5 ml-1">
                  <TrendingUp className="h-3 w-3" /> Amount (ILS)
                </label>
                <input name="amount" type="number" step="0.01" required placeholder="0.00" className="w-full rounded-xl border-none bg-white px-4 py-2.5 shadow-sm ring-1 ring-zinc-200 focus:ring-2 focus:ring-emerald-500 dark:bg-zinc-900 dark:ring-zinc-700" />
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <select
                  name="categoryId"
                  required
                  className="w-full rounded-xl border-none bg-white px-4 py-2.5 shadow-sm ring-1 ring-zinc-200 focus:ring-2 focus:ring-emerald-500 dark:bg-zinc-900 dark:ring-zinc-700"
                ...
                >
                  ...
                  <option value="new" className="font-bold text-emerald-600 dark:text-emerald-400">+ Add New Category</option>
                </select>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5 ml-1">
                ...
                <input name="date" type="date" defaultValue={defaultDate} required className="w-full rounded-xl border-none bg-white px-4 py-2.5 shadow-sm ring-1 ring-zinc-200 focus:ring-2 focus:ring-emerald-500 dark:bg-zinc-900 dark:ring-zinc-700" />
              </div>
            </div>
            ...
            <AnimatePresence>
              {isCreatingCategory && (
                ...
                >
                  <input name="newCategoryName" type="text" required placeholder="Category Name" className="flex-1 rounded-xl border-none bg-white px-4 py-2.5 shadow-sm ring-1 ring-zinc-200 focus:ring-2 focus:ring-emerald-500 dark:bg-zinc-900 dark:ring-zinc-700" />
                  <input name="newCategoryColor" type="color" defaultValue="#10b981" className="h-[46px] w-16 p-1.5 rounded-xl border-none bg-white shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 dark:ring-zinc-700" />
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="flex items-center justify-between pt-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative">
                  ...
                  <div className="h-6 w-11 rounded-full bg-zinc-200 transition peer-checked:bg-emerald-600 dark:bg-zinc-700" />
                  <div className="absolute left-1 top-1 h-4 w-4 rounded-full bg-white transition peer-checked:translate-x-5" />
                </div>
                <span className="text-sm font-bold text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-200">Recurring Income</span>
              </label>
              <button type="submit" className="rounded-xl bg-zinc-900 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-zinc-900/20 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-200">
                {editingId ? "Update Entry" : "Save Income"}
              </button>
            </div>
          </motion.form>
        )}
        ...
        <ul className="grid gap-3">
          {incomes.map((item) => (
            <motion.li
              ...
              className="group flex flex-col gap-3 rounded-2xl bg-zinc-50/50 p-4 transition-all hover:bg-white hover:shadow-md dark:bg-zinc-800/20 dark:hover:bg-zinc-800/40 ring-1 ring-transparent hover:ring-emerald-500/20"
            >
              {editingId === item.id ? (
                <form onSubmit={(e) => { handleSubmit(e); setEditingId(null); }} className="space-y-3">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button type="submit" className="rounded-xl bg-emerald-600 px-4 py-1.5 text-sm font-bold text-white">Save</button>
                  </div>
                </form>
              ) : (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm font-semibold text-zinc-500 dark:text-zinc-400">
                        {item.notes || <span className="italic opacity-50 font-normal">Source unknown</span>}
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => handleDelete(item.id)} className="rounded-lg p-2 text-zinc-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                </div>
              )}
      ...
      {!incomes.length && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm font-bold text-zinc-400 uppercase tracking-widest">No incomes found</p>
        </div>
      )}
    </div>
  );
